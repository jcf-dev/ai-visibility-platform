...
class Orchestrator:
    ...
    async def process_run(self, run_id: int, models: list[str]):
        ...
        async with self.db_session_factory() as session:
            ...
            for prompt in prompts:
                for model in models:
                    tasks.append(self._process_single_prompt(run_id, prompt, model, brands))
    ...
    async def _process_single_prompt(self, run_id: int, prompt: Prompt, model: str, brands: list[Brand]):
        async with self.semaphore:
            # We create a NEW session for each task or small batch to avoid long-lived session issues
            # and to allow partial commits.
            async with self.db_session_factory() as session:
                try:
                    # Call LLM
                    ...
                    async with self.db_session_factory() as err_session:
                         # Re-fetch prompt/run context if needed, or just insert a failed response
